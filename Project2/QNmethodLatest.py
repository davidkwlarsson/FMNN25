...
class General:
    ...
    def __call__(self):
        ...
        for i in range(10000):
            ...
            if type(s) is str:
                ...
                return self.pos, self.f(self.pos), 'curve to step, we give up'
    ...
    def getGradient(self, newpos):
        if(self.exactg):
            g = sc.array([i(newpos)for i in self.gfunc])
            return g
        h=1e-6
...
def main():
    ...
    rg1 = lambda x: 400*(x[0]**3)-400*x[0]*x[1]-2+2*x[0]
    rg2 = lambda x:200*x[1]-200*(x[0]**2)
    rgrad = [rg1,rg2]
    # if  lineSearch == True -> inexact method
    #qn = Newton(infunc, g1,[0.8,1.1], True, None)
    
    infunc = lambda x: x[0]**2 + (x[1]-5)**2
    g1 = lambda x: 2*x[0]
    g2 = lambda x: 2*(x[1]-5)
    grad= [g1,g2]
    qn = Broyden(rosen, [10,10], False)
